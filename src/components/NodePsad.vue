          ...
          <v-card color="blue-grey darken-2" class="white--text" >
            <v-card-title primary-title>
              ...
              <div>
                ...
                <h2 class="headline mb-1">P{{namenode}}</h2>
              </div>
            </v-card-title>
          </v-card>
        </v-flex>
...
export default {
  ...
  async mounted () {
    await this.$bindAsArray('todos', Data, null, () => {
      ...
      var vm = this
      let getData = this.todos.find(datas => datas.node === 'Nodesad')
      vm.value = getData
    ...
    await this.$bindAsArray('todoscheck', alive, null, () => {
      this.nodeonline = this.todoscheck.find(datas => datas.nodeName === 'Nodesad')
    })
  },
  data () {
    return {
  ...
  methods: {
    ...
    inboundLimit () {
      // var key = this.dataTouse.find(datas => datas.node === 'Nodesad')
      if (this.inbo !== '') {
