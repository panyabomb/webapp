...
export default {
  ...
  async mounted () {
    await this.$bindAsArray('todos', Data, null, () => {
      ...
      var vm = this
      let getData = this.todos.find(datas => datas.node === 'Node330A')
      vm.value = getData
    ...
    await this.$bindAsArray('todoscheck', alive, null, () => {
      this.nodeonline = this.todoscheck.find(datas => datas.nodeName === 'Node330A')
    })
  },
  data () {
    return {
  ...
  methods: {
    ...
    inboundLimit () {
      // var key = this.dataTouse.find(datas => datas.node === 'Node330A')
      if (this.inbo !== '') {
